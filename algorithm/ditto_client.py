...
class Client:
    ...
    def classify_training(self, lr):
        ...
        # Then train the client's local model
        print("Train personalized model....")
        p_model = self.personalized_model
        p_model.train()
        p_model = p_model.cuda()
        p_optimizer = optim.Adam(params=filter(lambda p: p.requires_grad, p_model.parameters()), lr=lr)
        for epoch in range(iterations):
            len_dataloader = len(tmp_train_loader)
            data_source_iter = iter(tmp_train_loader)
            for i in range(len_dataloader):
                ...
                class_output = p_model(input_data=s_img)
                err_s_label = loss_class(class_output, s_label)
                difference = self.model_difference(startPoint, p_model)
