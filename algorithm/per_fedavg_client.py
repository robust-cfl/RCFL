...
class Client:
    def __init__(self, train_loader, test_loader, model, hp, id_num):
        ...
        self.tmp_train_loader = None
        self.data_source_iter = None
    ...
    def classify_training(self, lr):
        ...
        if self.hp.dp:
            ...
            tmp_train_loader = self.train_loader
        self.tmp_train_loader = tmp_train_loader
        self.data_source_iter = iter(self.tmp_train_loader)
        for epoch in range(iterations):
            len_dataloader = len(self.tmp_train_loader)
            for i in range(len_dataloader):
                temp_model = deepcopy(model).state_dict()
                # step 1
                optimizer.zero_grad()
                # training model using source data
                data_source = self.get_next_train_batch()
                s_img, s_label = data_source
                # move to gpu
                s_img, s_label = s_img.cuda(), s_label.cuda()
    ...
    def get_next_train_batch(self):
        try:
            ...
            (X, y) = next(self.data_source_iter)
